...
export default function Home({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const viviendaUrl = `/api/residencias/vivienda/${id}/`;
  const listUrl = `/api/residencias/vivienda/`;
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR<ViviendaGet>(shouldFetch ? viviendaUrl : null, apiFetcher);
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        `¿Estás seguro de que quieres eliminar a ${vivienda?.nro_vivienda} ? Esta acción no se puede deshacer.`
      )
    ) {
      return;
    }
  ...
  return (
    <div className="max-w-xl mx-auto p-4 sm:p-6 bg-white shadow-lg rounded-lg">
      <div className="flex justify-between items-center border-b pb-2 mb-4">
        ...
        <Button2
          href={`/admin/residencias/viviendas/${vivienda.id}/editar`}
          size="md"
          variant="update"
        >
          <EditIcon /> Editar
      ...
      <div className="space-y-3">
        ...
        <DetailItem
          label="Estado"
          value={vivienda.estado ? "Ocupada" : "Disponible"}
          colorClass={
            vivienda.estado
        ...
        <DetailItem
          label="Alquiler"
          value={
            vivienda.precio_alquiler ? `$${vivienda.precio_alquiler}` : "N/D"
          }
