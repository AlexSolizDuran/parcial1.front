...
export default function Home({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { id } = use(params)
    const contratoUrl = `${url}/residencias/contrato/${id}/micontrato/`
    const deleUrl = `${url}/residencias/contrato/${id}/`

    const router = useRouter()
    const [isDeleting, setIsDeleting] = useState(false);
    const [deleteError, setDeleteError] = useState<string | null>(null);

    const { data: micontrato, error, isLoading } = useSWR<MiContrato>(contratoUrl, apiFetcher)
    ...
    const handleDelete = async () => {
        if (!window.confirm(`¿Seguro de eliminar el contrato de ${inquilino?.usuario_detail?.persona.nombre}?`)) {
            return;
        }
        setIsDeleting(true);
        setDeleteError(null);
        ...
        <div className="max-w-2xl mx-auto p-4 bg-white shadow-xl rounded-lg mt-6">
            ...
            <div className="flex justify-between items-center border-b pb-3 mb-4">
                <h1 className="text-xl font-bold text-gray-800">
                    Contrato: {inquilino?.usuario_detail?.persona.nombre} {inquilino?.usuario_detail?.persona.apellido}
                </h1>
                <div className="flex space-x-2">
                    <Button2
                        href={`/admin/residencias/contratos/${contrato?.id}/editar`}
                        size="sm" variant="update"
                    >
                        <EditIcon /> Editar
                    </Button2>
                    <ButtonDelete
            ...
            <div className="space-y-2 mb-4 p-3 border rounded-lg bg-blue-50">
                <DetailItem label="Nro. Vivienda" value={String(contrato?.vivienda_detail.nro_vivienda)} />
                <DetailItem label="Tipo de Renta" value={String(contrato?.tipo_renta)} />
                <DetailItem label="Expensa" value={`${String(contrato?.porcentaje_expensa)}%`} />
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="p-3 border rounded-lg bg-green-50">
                    ...
                    <p className="text-lg font-bold text-green-700">{formatDate(String(contrato?.fecha_ingreso))}</p>
                </div>
                <div className="p-3 border rounded-lg bg-red-50">
                    <p className="font-semibold text-sm text-gray-600">Fin</p>
                    <p className="text-lg font-bold text-red-700">{formatDate(String(contrato?.fecha_ingreso))}</p>
                </div>
            </div>
            ...
            <div className="space-y-2 mb-4 p-3 border rounded-lg">
                ...
                <DetailItem label="Nombre Completo" value={String(inquilino?.usuario_detail?.persona.nombre) + " " + String(inquilino?.usuario_detail?.persona.apellido)} colorClass="font-bold" />
                <DetailItem label="Email Inquilino" value={String(inquilino?.usuario_detail?.email)} />
                <DetailItem label="CI Inquilino" value={String(inquilino?.usuario_detail?.persona.ci)} />
                <DetailItem label="Telefono Inquilino" value={String(inquilino?.usuario_detail?.persona.telefono)} />
                <Button2 href={`/admin/usuarios/usuarios/${inquilino?.id}`} size="md" variant="show"> Ver</Button2>
                ...
                <>
                    ...
                    <div className="space-y-2 mb-4 p-3 border rounded-lg bg-yellow-50">
                        {ocupantes?.map((ocupante, index) => (
                            <div key={ocupante.id || index} className="border-b border-yellow-200 last:border-b-0 py-2">
                                ...
                                {ocupante.persona ? (
                                    <>
                                        <DetailItem
                                            ...
                                            value={`${ocupante.persona.nombre || ''} ${ocupante.persona.apellido || ''}`}
                                        ...
                                        />
                                        <Button2 href={`/admin/usuarios/otros/${ocupante.persona.id}`} size="sm" variant="show"> Ver</Button2>

                                    </>

                                ) : (
                                    <p className="text-sm text-red-600 mt-1">⚠️ Detalles de persona no disponibles.</p>
                                )}
