...
export default function UsuarioDetail({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { data: usuario, error, isLoading } = useSWR<Usuario>(
        usuarioUrl,
        apiFetcher
    );
    ...
    const handleDelete = async () => {
        if (!window.confirm(`¿Estás seguro de que quieres eliminar a ${usuario.persona?.nombre} ${usuario.persona?.apellido}? Esta acción no se puede deshacer.`)) {
            return;
        }
        ...
        try {
            await apiFetcher(usuarioUrl, {
                method: 'DELETE',
            });
            mutate(listUrl);
            router.push("/admin/usuarios/usuarios?deleted=true");
        } catch (err: any) {
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6">
                ...
                <div className="flex-shrink-0 w-24 h-24 rounded-full bg-blue-600 flex items-center justify-center border-4 border-white shadow-md">
                    {usuario.persona.foto ? (
                        <img src={usuario.persona.foto} className="w-full h-full rounded-full object-cover" />
                    ) : (
                        <span className="text-4xl font-bold text-white">{usuario.persona.nombre?.charAt(0).toUpperCase()}</span>
                    )}
                </div>

                {/* Información Principal */}
                <div className="flex-1 text-center sm:text-left">
                    <h1 className="text-3xl font-bold text-gray-800">{usuario.persona.nombre} {usuario.persona.apellido}</h1>
                </div>
                {/* Botones de Acción */}
                <div className="flex items-center gap-3">
                    <Button2
                        href={`/admin/usuarios/usuarios/${usuario.id}/editar`} size="md" variant="update"
                    >
                        <EditIcon /> Editar
                    </Button2>
                ...
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-8">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <dd className="text-md text-gray-900">{usuario.username || 'No especificado'}</dd>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <EmailIcon />
                        <div>
                            ...
                            <dd className="text-md text-gray-900">{usuario.email || 'No especificado'}</dd>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <IdCardIcon />
                        <div>
                            ...
                            <dd className="text-md text-gray-900">{usuario.persona.ci || 'No especificado'}</dd>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <PhoneIcon />
                        <div>
                            ...
                            <dd className="text-md text-gray-900">{usuario.persona.telefono || 'No especificado'}</dd>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <CalendarIcon />
                        <div>
                            ...
                            <dd className="text-md text-gray-900">{usuario.persona.fecha_nacimiento ? new Date(usuario.persona.fecha_nacimiento).toLocaleDateString() : 'No especificado'}</dd>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <GenderIcon />
                        <div>
                            ...
                            <dd className="text-md text-gray-900 capitalize">{usuario.persona.genero || 'No especificado'}</dd>
                        </div>
                    </div>
